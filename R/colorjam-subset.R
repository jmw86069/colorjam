 ...
 return_type=c("colors", "df"))
{
   ...
   x_df <- data.frame(num=seq_along(x),
      hex=jamba::rgb2col(grDevices::col2rgb(x, alpha=TRUE)),
      t(jamba::col2hcl(x)),
      t(grDevices::col2rgb(x, alpha=FALSE)),
      t(jamba::col2hsv(x, alpha=FALSE))[,c("h","s","v")]);
   x_df_sub <- subset(x_df, ...);
   if (length(byCols) > 0 && any(byCols %in% colnames(x_df))) {
      x_df_sub <- jamba::mixedSortDF(x_df_sub,
         byCols=byCols);
   }
   if ("df" %in% return_type) {
      return(x_df_sub);
   }
   if (length(x_df_sub) == 0) {
      return(character(0))
   }
   return(x[x_df_sub$num]);
}
